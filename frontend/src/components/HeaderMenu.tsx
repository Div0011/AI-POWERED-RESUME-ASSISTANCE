...
import {
    ...
    Settings, LogOut, Search, Plus, Mic, Upload, Menu, X, Globe
} from 'lucide-react';
import { ThemeToggle } from './ui/ThemeToggle';

const recruiterNav = [
    { name: 'Talent Matrix', icon: Users, path: '/recruiter/dashboard' },
    { name: 'Global Talent', icon: Globe, path: '/recruiter/talent-pool' },
    { name: 'Post Job', icon: Plus, path: '/recruiter/jobs/new-enhanced' },
...
export const HeaderMenu = () => {
    ...
    let menuItems = candidateNav; // Default

    if (isRecruiterPath) {
        menuItems = recruiterNav;
    } else if (isCandidatePath) {
        menuItems = candidateNav;
    } else {
        // Fallback to role-based
        menuItems = user?.role === 'recruiter' ? recruiterNav : candidateNav;
    }
    ...
    const getCmdLabel = () => {
        if (pathname.includes('/interview')) return 'INTERVIEW_SIMULATION_ACTIVE';
        ...
        <>
            <AnimatePresence mode="wait">
                {!isHovered && !isOpen ? (
                    ...
                    >
                        <div className="glass-panel px-6 py-2 rounded-full border border-[var(--primary)]/20 bg-[var(--obsidian)]/60 backdrop-blur-md shadow-[0_4px_20px_rgba(0,0,0,0.1)] flex flex-col items-center gap-1 hover:bg-[var(--primary)]/10 hover:border-[var(--primary)]/40 hover:scale-105 transition-all duration-300 group">
                            <div className="w-8 h-[2px] bg-[var(--foreground)]/30 group-hover:bg-[var(--primary)] transition-colors rounded-full" />
                            <span className="text-[8px] font-black tracking-[0.4em] text-[var(--foreground)]/50 group-hover:text-[var(--primary)] transition-colors uppercase">Menu</span>
                        </div>
                    </motion.div>
                ) : (
                    <motion.header
                            ...
                            {menuItems.map((item) => {
                                ...
                                return (
                                    <Link
                                        key={item.path}
                                        href={item.path}
                                        className={`relative px-3 py-2 rounded-lg text-xs md:text-sm font-medium transition-all group whitespace-nowrap ${isActive ? 'text-[var(--foreground)]' : 'text-[var(--foreground)]/60 hover:text-[var(--foreground)]'}`}
                                    >
